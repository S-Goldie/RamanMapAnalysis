...
__comment__ = "Version4.1, adapt data read to account for Renishaw style sequential map data"
        ...
        try:
            with open(input_name, 'r') as fin:
                if '.csv' in input_name:
                    cin = csv.reader(fin)
                    datalist = [row for row in cin]
                elif '.txt' in input_name:
                    cin = csv.reader(fin, delimiter='\t')
                    datalist = [row for row in cin]
                datalist[0] = list(filter(None, datalist[0]))
                
                if len(datalist[0]) > 4:
                    shortname = input_name[:(input_name.find('.'))]
                    longname = input_name
                    ...
                    longname = input_name
                    point_index = []
                    for r in range(1,len(datalist)-1):
                        if float(datalist[r+1][0]) - float(datalist[r][0]) != 0:
                            point_index.append(r)       #generate list of coordinates that are the final datapoints for each spectrum
                    point_index.append(len(datalist))

                    data = np.zeros([len(point_index)+1,int(point_index[2]-point_index[1])],dtype=float)
                    for r in range(1,int(point_index[2]-point_index[1])+1):
                        data[0,r-1]=float(datalist[r][2])
                    
                    row = 1
                    column = 0

                    for r in range(1,len(datalist)):
                        data[row,column] = float(datalist[r][3])
                        if r in point_index:
                            row += 1
                            column = 0
                        else:
...
if rank < 20:
    plt.hist2d(hist_x,hist_y, cmin=1)
else:
